...
> **注意**：这是一个展示性的项目，仅用于展示项目的设计思路和完成度，**无法直接使用**。完整代码暂时没有开源，目前的代码文件只截取了部分非核心逻辑展示，作者在进行必要操作（部分问题和功能的修补、软著申请）后，大约一个月左右或者更短的时间内（2026.7.1之前），进行完整项目的开源。

基于 LLM 的 TRPG（桌上角色扮演游戏）KP 助手，COC 7th 规则。从玩家输入到沉浸式叙事生成外加辅助工具的完整调用链。

> **项目综述**：本项目是一个完全由 AI 驱动的项目，除了项目的底层能力来自大模型调用外，此外约 98% 的代码由 LLM 自动生成，约 50% 的测试和测试 log 审计工作也由大模型完成。作者主要负责需求分析、架构设计、提示词微调、方法论管理与测试补充和验证。
...
**统计**：完整管线共约 13 次 LLM 调用（Step 0 × 1 + Step 1 × 2 + Step 2 × 4 + Step 3 × 2 + Step 3.5 × 1 + Phase 2 × 1 + Boss 补充 × 1），支持断点续跑和手动/自动模式。
...
| **L3 设计者层** | Author | 世界规则、叙事线、时间压力、结局条件、创作者豁免(WRO) | 动态创作 |
...
| Agent | 模型 | 职责 |
|-------|------|------|
| **Keeper（主控）** | Pro + Flash | 回合编排中枢，持有 Judge/Curator/CombatSystem 等确定性子系统 |
| **Narrator（叙事者）** | Pro | 将结构化结果转化为沉浸式叙事文本 |
| **Author（创作者）** | Pro | 动态扩展：当玩家行为超出预设时，动态生成新实体/场景 |
...
| U9 | **技能系统重修**：COC 7 规则技能体系本身抽象，不必照搬 | 高 |
| U3 | LLM Provider 抽象：支持 OpenAI/Anthropic 多 provider | 中 |
| U4 | 跨模组持久化：调查员永久化、战役系统（依赖技能系统重修） | 中 |
| U6 | 法术体系：SpellJudge + @grant_spell（依赖世界状态系统+技能系统重修） | 中 |
| U7 | LLM 调用成本优化：各步骤最低配置 | 中 |
| U8 | 物品系统升级 | 中低 |
| U1 | NPC 系统升级：态度、跟随系统更主动化（等另一个项目技术迁移） | 低 |
| U10 | 自动化测试体系：已完成基本版，剩余部分等另一个项目技术迁移 | 低 |
| U5 | 多人模式 (Hotseat)：同机多调查员，系统完善前不考虑 | 极低 |
