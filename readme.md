...
基于 LLM 的 TRPG（桌上角色扮演游戏）KP 助手。这是一个让 AI 完整跑完 TRPG 游戏全部流程的系统，AI创作剧本、实时理解玩家行动、掷骰判定，还能动态创作原本不存在的剧情分支。工程上为了解决传统AI角色扮演游戏模型幻觉、数值混乱和 Token 爆炸的问题，设计了“混血”架构融合使用大模型+自然语言和确定性代码将单个 Agent 上下文控制在 2000 token 以内；并通过 L1/L2/L3 三层数据解耦与标准消息合约进一步规范了多agent系统的协作模式。
