...
本项目是一个由大模型完全驱动的 TRPG（桌上角色扮演游戏）全流程守秘人（KP）引擎。系统实现了从 AI 离线创作剧本、实时理解玩家意图、执行确定性规则判定，到动态构建未知剧情分支的完整闭环。工程上为了解决传统AI角色扮演游戏模型幻觉、数值混乱和 Token 爆炸的问题，设计了“混血”架构融合使用大模型+自然语言和确定性代码在测试模组上成功将单个 Agent 的单轮上下文完美控制在 2000 token 以内，规范了多 Agent 系统的健壮协作模式。
